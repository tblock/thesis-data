...
class CLSHyperParams(LMHyperParams):
    ...
    def train_cls(self, num_lm_epochs, unfreeze=True, bs=16, true_wd=True, drop_mul_lm=0.3, drop_mul_cls=0.5, use_test_for_validation=False):
        ...
        if true_wd:
            ...
            print("Starting lowshot training")
            learn.freeze_to(-1)
            learn.fit_one_cycle(1, lrs, moms=(0.8, 0.7), wd=1e-7)
            learn.freeze_to(-2)
            learn.fit_one_cycle(1, lrs, moms=(0.8, 0.7), wd=1e-7)
            learn.freeze_to(-3)
            learn.fit_one_cycle(1, lrs, moms=(0.8, 0.7), wd=1e-7)
            learn.unfreeze()
            learn.fit_one_cycle(4, lrs, moms=(0.8, 0.7), wd=1e-7)
        else:
            learn.true_wd = False
        ...
        result = self.validate_cls('cls_last')[1]
        print("#######################")
        print("%.2f" % (float((1-result)*100)))
        return None
    ...
    def load_cls_data_imdb(self, bs, force=True, use_test_for_validation=False):
        trn_df = pd.read_csv(self.dataset_path / 'train.csv', header=None, sep=';', quotechar="'")
        tst_df = pd.read_csv(self.dataset_path / 'test.csv', header=None, sep=';', quotechar="'")
        unsp_df = pd.read_csv(self.dataset_path / 'unsup.csv', header=None, sep=';', quotechar="'")

        lm_trn_df = pd.concat([unsp_df, trn_df, tst_df])
        val_len = max(int(len(lm_trn_df) * 0.2), 2)
