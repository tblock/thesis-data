...
class CLSHyperParams(LMHyperParams):
    ...
    def train_cls(self, num_lm_epochs, unfreeze=True, bs=16, true_wd=True, drop_mul_lm=0.3, drop_mul_cls=0.5,
                  use_test_for_validation=False):
        ...
        if true_wd:
            ...
            print("Starting classifier training without wd")
            learn.freeze_to(-1)
            ...
            learn.fit_one_cycle(1, lrs, moms=(0.8, 0.7))
            learn.unfreeze()
            learn.fit_one_cycle(4, lrs, moms=(0.8, 0.7))
        else:
            learn.true_wd = False
    ...
    def validate_cls(self, save_name='cls_last', bs=20):
    ...
    def load_cls_data(self, bs, **kwargs):
        if 'ompc' in self.dataset_dir.name:
